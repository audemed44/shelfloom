...
function GenreCombobox({
  ...
  const createAndAdd = useCallback(
    async (name: string) => {
      ...
      try {
        ...
        if (created) await addGenre(created)
      } catch {
        const existing = allGenres.find(
          (g) => g.name.toLowerCase() === name.toLowerCase()
        )
        if (existing) await addGenre(existing)
      } finally {
        setBusy(false)
      }
    },
  ...
  const removeGenre = useCallback(
    async (genre: Genre) => {
      setBusy(true)
      try {
        await api.delete(`/api/books/${bookId}/genres/${genre.id}`)
    ...
    <div>
      ...
      >
        ...
        <input
          ...
          placeholder={genres.length === 0 ? 'Type to search or add…' : ''}
